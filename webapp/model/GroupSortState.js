sap.ui.define([
	...
	return BaseObject.extend("ZFER02_FRAGMENTS.ZFER02_FRAGMENTS.model.GroupSortState", {
		 ...
		 * @alias ZFER02_FRAGMENTS.ZFER02_FRAGMENTS.model.GroupSortState
		 */
		...
		/**
		 * Sorts by TaskTitle, or by PriorityNumber
		 *
		...
		/**
		 * Groups by PriorityNumber, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "PriorityNumber") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "PriorityNumber");

				aSorters.push(
					new Sorter("PriorityNumber", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "TaskTitle");
			}
