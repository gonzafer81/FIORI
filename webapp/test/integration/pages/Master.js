			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("InstanceID") === oObject.InstanceID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].InstanceID;
									} else {
										sCurrentId = aItemsNotInTheList[0].InstanceID;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("TaskCollection", {
										InstanceID : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("InstanceID"),
						title: oBindingContext.getProperty("TaskTitle")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<TaskCollection> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("PriorityNumber");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("TaskTitle");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all TaskCollection to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<TaskCollection> (" + iExpectedLength + ")"}),
								success : function () {
